...
public class Emojis {
    private static final Color
        ...
        EYE_FILL_COLOR = new Color(0x000000);

    public static void main(String[] args) {
        CanvasWindow canvas = new CanvasWindow("Emojis", 800, 600);

        GraphicsGroup littleSmiley = createSmileyFace(100, 10);
        littleSmiley.setPosition(50, 400);
        canvas.add(littleSmiley);

        GraphicsGroup mediumSmiley = createSmileyFace(200,20);
        mediumSmiley.setPosition(150, 300);
        canvas.add(mediumSmiley);

        GraphicsGroup bigSmiley = createSmileyFace(300,30);
     ...
     */
    public static GraphicsGroup createSmileyFace(double size, double sizeEyes) {
        ...
        Ellipse eye1 = createEye(sizeEyes);
        eye1.setCenter(size * 0.25, size * 0.25);
        group.add(eye1);

        Ellipse eye2 = createEye(sizeEyes);
        eye2.setCenter(size * 0.75, size * 0.25);
        group.add(eye2);
    ...
    /**
     * Creates a winking face emoji.
     *
     * @param size The overall width and height of the emoji.
     * @return A graphic that you can add to a window, or place inside some other graphics group.
     */
    public static GraphicsGroup createWinkingFace(double size, double sizeEyes) {
        GraphicsGroup group = new GraphicsGroup();
    ...
    private static Ellipse createEye(double diameter) {
        Ellipse eye = new Ellipse(0, 0, diameter, diameter);
        eye.setFillColor(EYE_FILL_COLOR);
        eye.setStrokeColor(EYE_FILL_COLOR);
        eye.setStrokeWidth(2);
        return eye;
    }

    private static Rectangle createClosedEye(int width, int length){
        Rectangle closedEye = new Rectangle(0,0, width, length);
        //closedEye.setFillColor();
        //closedEye.setStrokeColor(EYE_FILL_COLOR);
        //closedEye.setStrokeWidth(2);
        return closedEye;
    }
